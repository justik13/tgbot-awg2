from typing import Optional, List
import datetime

class DevicesMixin:
    async def add_device(self, user_id: int, server_id: int, name: str, amnezia_client_id: str, config_text: str):
        encrypted_config_text = self._encrypt(config_text)
        created_at = datetime.datetime.now().isoformat()
        await self.connection.execute('''
            INSERT INTO devices (user_id, server_id, name, amnezia_client_id, config_text, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (user_id, server_id, name, amnezia_client_id, encrypted_config_text, created_at))
        await self.connection.commit()

    async def get_device(self, device_id: int) -> Optional[dict]:
        cursor = await self.connection.execute('SELECT * FROM devices WHERE id = ?', (device_id,))
        row = await cursor.fetchone()
        if row:
            decrypted_config_text = self._decrypt(row['config_text'])
            return {**row, 'config_text': decrypted_config_text}
        return None

    async def get_user_devices(self, user_id: int) -> List[dict]:
        devices = []
        cursor = await self.connection.execute('SELECT * FROM devices WHERE user_id = ?', (user_id,))
        rows = await cursor.fetchall()
        for row in rows:
            decrypted_config_text = self._decrypt(row['config_text'])
            devices.append({**row, 'config_text': decrypted_config_text})
        return devices

    async def get_user_devices_count(self, user_id: int) -> int:
        cursor = await self.connection.execute('SELECT COUNT(*) FROM devices WHERE user_id = ?', (user_id,))
        row = await cursor.fetchone()
        return row[0] if row else 0

    async def delete_device(self, device_id: int):
        await self.connection.execute('''
            DELETE FROM devices
            WHERE id = ?
        ''', (device_id,))
        await self.connection.commit()

    async def create_temporary_link(self, device_id: int, token: str, expires_at: str):
        await self.connection.execute('''
            INSERT INTO temporary_links (device_id, token, expires_at)
            VALUES (?, ?, ?)
        ''', (device_id, token, expires_at))
        await self.connection.commit()

    async def get_device_by_temporary_token(self, token: str) -> Optional[dict]:
        cursor = await self.connection.execute('''
            SELECT * FROM temporary_links WHERE token = ?
        ''', (token,))
        row = await cursor.fetchone()
        if row:
            expires_at = datetime.datetime.fromisoformat(row['expires_at'])
            if expires_at > datetime.datetime.now():
                device_cursor = await self.connection.execute('SELECT * FROM devices WHERE id = ?', (row['device_id'],))
                device_row = await device_cursor.fetchone()
                if device_row:
                    decrypted_config_text = self._decrypt(device_row['config_text'])
                    return {**device_row, 'config_text': decrypted_config_text}
        return None
